...
- Feature/product implementation: use an end-state skeleton for accepted future capabilities; 1% callable stubs, hooks, slots, or contracts are better than absent architecture when the product direction is known. Verify object inventory against the final product plan before judging detail depth.
