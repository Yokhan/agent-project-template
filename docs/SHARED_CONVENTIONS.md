...
Last reviewed: 2026-07-04 for template `4.4.2`; product/business outcome priority, client/executor accountability, progressive JPEG delivery, text policy, platform policy, CI runtime hygiene, production design QA, register-aware design pipeline checks, screen anatomy/root-frame checks, design-policy fixture sync delivery, git dry-run sync preview, and agent-safe GitHub release entrypoint checks are enforced by validators.
