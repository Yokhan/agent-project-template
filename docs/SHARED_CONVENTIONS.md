...
Last reviewed: 2026-07-10 for template `4.6.1`; product/business outcome priority, client/executor accountability, progressive JPEG delivery plus final-plan object readiness, progressive layer replacement, `PROGRESSIVE_STATUS` project-slice reporting, semantic intent routing, adaptive GPT-5.6 fan-out, canonical pinned-tag updates, text policy, platform policy, CI runtime hygiene, production design QA, register-aware design pipeline checks, screen anatomy/root-frame checks, design-policy fixture sync delivery, git dry-run sync preview, and agent-safe GitHub release entrypoint checks are enforced by validators.
